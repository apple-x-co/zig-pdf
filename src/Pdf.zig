...
pub fn init(author: ?[]const u8, creator: ?[]const u8, title: ?[]const u8, subject: ?[]const u8, compression_mode: ?CompressionMode, owner_password: ?[]const u8, user_password: ?[]const u8, encryption_mode: ?[]const u8, encryption_length: ?u32) Self {
    return .{
        .author = author,
        .creator = creator,
        .title = title,
...
test {
    const pdf = init("apple-x-co", "zig-pdf", "demo", "demo1", CompressionMode.all, null, null, "Revision2", null);
